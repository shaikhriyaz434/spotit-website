...
let OrgFormComponent = class OrgFormComponent {
    constructor(http) {
        this.http = http;
        this.orgFormData = {};
    }
    ...
    submit(data) {
        ...
        this.http.postMethod(this.http.organizationRoute, this.orgFormData).subscribe(data => {
            // console.log(data);
            this.initData();
        });
    }
};
OrgFormComponent.ctorParameters = () => [
...
let OrgunitFormComponent = class OrgunitFormComponent {
    constructor(http, position, fb) {
        this.http = http;
    ...
    ngOnInit() {
        ...
        this.http.getMethod(this.http.organizationRoute).subscribe(data => {
            // console.log(data);
            this.orgAutoOptions = JSON.parse(JSON.stringify(data));
        });
    ...
    _filter(value) {
        if (typeof value === 'string') {
            ...
            return this.orgAutoOptions.filter((option) => {
                return option.desc.toLowerCase().includes(filterValue);
            });
        }
        else {
    ...
    getPosition() {
        // this.position.
        this.position.getCurrentPosition().then((location) => {
    ...
    submit(data) {
        this.position.getCurrentPosition().then((location) => {
            ...
            this.http.postMethod(this.http.orgUnitRoute, payload).subscribe(data => {
                this.clearForm();
            });
...
OrgunitFormComponent.ctorParameters = () => [
    { type: _services_http_service__WEBPACK_IMPORTED_MODULE_2__["HttpService"] },
